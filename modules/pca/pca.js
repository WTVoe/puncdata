...
var PCA = (function () {
    ...
    function computeDeviationMatrix(matrix) {
        var unit = unitSquareMatrix(matrix[0].length);
        var ret = subtract(transpose(matrix), scale(multiply(unit, transpose(matrix)), 1 / matrix[0].length))
    ...
    function computeSVD(matrix) {
        ...
        var results = eigenvalues.map(function (value, i) {
            ...
            obj.vector = eigenvectors.map(function (vector, j) {
                return vector[i]; //HACK prevent completely negative vectors
            });
            return obj;
        });
        return results;
    ...
    function getEigenVectors(data) {
        // return computeSVD(computeVarianceCovariance(computeDeviationScores(data), false));
    ...
    /**
     * Multiplies AxB, where A and B are matrices of nXm and mXn dimensions !!! warining, does not multiply in the usual direction!!!
     * @param {} a 
     * @param {*} b 
     */
